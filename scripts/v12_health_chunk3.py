...
OUTPUT_FILE = "/home/yoli/GitHub/web_nav_v2/data/v12_health_chunk3.json"
START_IDX = 3182
END_IDX = 4773
CONCURRENT_LIMIT = 12
TIMEOUT = 4
...
async def main():
    ...
    print(f"Chunk3: {len(queue)}站, 健康{len(healthy)}, 坏链{len(broken)}")

    with open(OUTPUT_FILE, 'w') as f:
        json.dump({"range": [START_IDX, END_IDX], "healthy": list(healthy), "broken_count": len(broken)}, f)
    print(f"✓ Chunk3 完成, 用时: {time.time()-start_t:.1f}s")
