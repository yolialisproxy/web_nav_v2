...
function renderSites(sites, containerId = 'main-content') {
    ...
    sites.forEach(site => {
        ...
        html += `
            <a href="${linkUrl}" class="site site-card" ${linkTarget} ${relAttr} data-title="${site.title || ''}">
                <img src="https://www.google.com/s2/favicons?domain=${site.url ? new URL(site.url).hostname : 'example.com'}&sz=32" class="card-icon" onerror="this.onerror=null;this.src='assets/images/favicon.png';">
                <strong class="card-title">${site.title || 'Untitled'}</strong>
                <span class="card-desc">${site.description || site.url || ''}</span>
            </a>
        `;
    });
    html += '</div>';
    ...
    _bindSidebarEvents() {
        this.sidebarContent.querySelectorAll('.menu-category').forEach(el => {
            el.addEventListener('click', (e) => {
                ...
                // Menu_System.md: 点击大类自动选中第一个中类和叶子
                const categories = dataManager.categories;
