...
(function () {
    ...
    if (!window.favoriteUI) {
        window.favoriteUI = {
            ...
            // ===== 渲染列表 =====
            renderList: function () {
                if (!this.favoriteList || !window.favoriteManager)
                    return;
                ...
                if (countEl)
                    countEl.textContent = 'Total: ' + count + ' items';
                if (count === 0) {
            ...
            _bindSearch: function () {
                ...
                if (!searchInput)
                    return;
                (searchInput as HTMLInputElement).value = this.searchQuery || '';
...
//# sourceMappingURL=favorite-ui-bootstrap.js.map