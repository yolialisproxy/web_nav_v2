...
var DatingGame = function () {
    window.GameEngine.call(this, { id: 'dating', title: '💕 恋爱大富翁' });
...
DatingGame.prototype.init = function () {
    window.GameEngine.prototype.init.call(this);
    // 尝试读取存档
    if (!this.load()) {
        this._selectMode();
    }
...
DatingGame.prototype._rollDice = function () {
    var steps = window.GameUtils.rand(1, 6);
    this.turns++;
...
DatingGame.prototype._processCell = function (cell) {
    var self = this;
    var targetIdx = window.GameUtils.rand(0, this.targets.length - 1);
    ...
    switch (cell.type) {
        ...
        case DatingGame.CELL_TYPES.RIVAL:
            ...
            window.GameUtils.playSound(150, 0.15, 'square');
            break;
...
DatingGame.prototype._giveGift = function () {
    ...
    overlay.querySelectorAll('[data-target]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            self.player.gold -= 20;
            var d = window.GameUtils.rand(8, 18);
...
DatingGame.prototype._checkWinLose = function () {
    ...
    this.targets.forEach(function (t) {
        if ((self.affection[t.name] || 0) >= 90) {
            self.state = 'over';
            self.gameOver();
            window.GameHub.showToast('💕 恭喜！' + t.icon + t.name + '接受了你的心意！最终得分: ' + self.score);
        }
    });
    if (this.player.gold <= 0 && this.day > 5) {
        this.state = 'over';
        self.gameOver();
        window.GameHub.showToast('💀 一贫如洗，感情之路也走到了尽头...');
    }
};
DatingGame.prototype.save = function () {
    ...
    window.GameUtils.save(this.saveKey, data);
};
DatingGame.prototype.load = function () {
    var data = window.GameUtils.load(this.saveKey);
    if (data) {
...
DatingGame.prototype.togglePause = function () {
    window.GameEngine.prototype.togglePause.call(this);
};
DatingGame.prototype.quit = function () {
    this.state = 'idle';
    window.GameHub.closeGame();
};
