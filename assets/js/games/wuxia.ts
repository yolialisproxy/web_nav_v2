...
WuxiaGame.prototype._bindEvents = function () {
    ...
    this.el.querySelectorAll('[data-choice]').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            if (choice.effect === 'heal_full')
                self.hp = self.maxHp;
            if (choice.effect === 'heal_half')
                self.hp = Math.min(self.maxHp, self.hp + Math.floor(self.maxHp / 2));
            if (choice.effect === 'attack+5')
                self.attack += 5;
            if (choice.effect === 'attack+10')
                self.attack += 10;
            if (choice.effect === 'defense+3')
                self.defense += 3;
            ...
            if (choice.next && (choice.next === 'fight_tiger' || choice.next === 'fight_boss' || choice.next === 'fight_boss_surprise')) {
                ...
                if (choice.reward && choice.reward.exp)
                    self.exp += choice.reward.exp;
    ...
    if (attackBtn)
        attackBtn.addEventListener('click', function () { self._playerAttack(); });
    var skillBtn = document.getElementById('wuxia-skill');
    if (skillBtn)
        skillBtn.addEventListener('click', function () { self._playerSkill(); });
    var defendBtn = document.getElementById('wuxia-defend');
    if (defendBtn)
        defendBtn.addEventListener('click', function () { self._playerDefend(); });
    var fleeBtn = document.getElementById('wuxia-flee');
    if (fleeBtn)
        fleeBtn.addEventListener('click', function () { self._playerFlee(); });
    // 新游戏
    var newGameBtn = document.getElementById('wuxia-new-game');
    if (newGameBtn)
        newGameBtn.addEventListener('click', function () { self.newGame(); });
};
...
WuxiaGame.prototype._playerSkill = function () {
    if (!this.fighting)
        return;
    if (this.exp < 30) {
        GameHub.showToast('SP不足！需要30EXP');
        return;
    }
    this.exp -= 30;
