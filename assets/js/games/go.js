...
GoGame.prototype._startTimer = function () {
    ...
    this._timerInterval = setInterval(function () {
        ...
        if (disp)
            disp.textContent = GoGame.prototype.formatTime(elapsed);
    }, 500);
};
...
GoGame.prototype._handleClick = function (e) {
    if (this.state !== 'running' || this.passCount >= 2)
        return;
...
GoGame.prototype._endGame = function () {
    ...
    this.gameOver();
    if (this.onGameOver)
        this.onGameOver(blackScore - whiteScore, Date.now() - this._timerStart);
};
/** 计算领地（简化：计算己方棋子及相邻空点） */
