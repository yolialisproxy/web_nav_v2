...
Game2048.prototype._move = function (direction) {
    ...
    this._saveState();
    var moved = false;
    var merged = Array.from({ length: this.size }, () => Array(this.size).fill(false));
    // 遍历顺序
...
Game2048.prototype._undo = function () {
    if (!this._history || this._history.length === 0) {
        GameHub.showToast('没有可撤销的步骤', 3000);
        return;
    }
...
Game2048.prototype._checkWin = function () {
    for (var r = 0; r < this.size; r++) {
        for (var c = 0; c < this.size; c++) {
            if (this.grid[r][c] === 2048) {
                ...
                this.gameOver();
                GameHub.showToast('🎉 恭喜达成2048！继续挑战更高分。', 3000);
                return;
            }
        }
    }
    ...
    if (this._isFull() && !this._canMove()) {
        this.gameOver();
        GameHub.showToast('💀 游戏结束！最终得分: ' + this.score, 3000);
    }
};
