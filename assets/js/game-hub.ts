...
var GameHub = {
    ...
    init: function () {
        ...
        if (toggle)
            toggle.addEventListener('click', function () { GameHub.openHub(); });
        if (close)
            close.addEventListener('click', function () { GameHub.closeHub(); });
        if (overlay)
        ...
        if (quitBtn)
            quitBtn.addEventListener('click', function () { GameHub.closeGame(); });
        // 渲染游戏网格
        this.renderGrid('all');
    },
    ...
    renderContainer: function () {
        // 如果容器已存在，跳过
        if (document.getElementById('game-play-overlay'))
            return;
    ...
    startGame: function (gameKey) {
        ...
        this.currentEngine = new (ctor as any);
    ...
    closeGame: function () {
        if (this.currentEngine) {
            if (this.currentEngine.save)
                this.currentEngine.save();
    ...
    toggleGameFav: function (key) {
        ...
        if (result && result.success) {
            // 重新渲染当前分类网格（更新所有按钮状态）
            if (this.currentCat)
                this.renderGrid(this.currentCat);
        }
    },
    ...
    pushRecent: function (key) {
        ...
        list.unshift(key);
        if (list.length > GameHub._RECENT_MAX)
            list.length = GameHub._RECENT_MAX;
        try {
            localStorage.setItem(GameHub._RECENT_KEY, JSON.stringify(list));
        }
        catch (e) { }
    },
...
//# sourceMappingURL=game-hub.js.map