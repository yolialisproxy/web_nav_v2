...
class TrieIndex {
    ...
    insert(text, siteId, weight = 1) {
        ...
        for (let i = 0; i < len; i++) {
            if (i > 0 && !this._isSeparator(normalized[i - 1]))
                continue;
            let node = this.root;
            for (let j = i; j < len; j++) {
...
class SearchEngine {
            ...
            if (site.tags && Array.isArray(site.tags)) {
                site.tags.forEach(tag => {
                    if (tag && typeof tag === 'string') {
                        ...
                        if (key) {
                            tagKeys.push(key);
                            if (!this._tagIndex.has(key))
                                this._tagIndex.set(key, new Set());
    ...
    query(text) {
        ...
        candidateIds.forEach(id => {
            ...
            if (score > 0) {
                results.push(Object.assign(Object.assign({}, site), { _score: score, _query: text }));
            }
        });
        // 3. 排序并返回前50条
        ...
        return results.slice(0, 50).map((_a) => {
            var { _score } = _a, site = __rest(_a, ["_score"]);
            return (Object.assign(Object.assign({}, site), { _query: text }));
        });
    }
    /**
     * 搜索建议（自动补全）
